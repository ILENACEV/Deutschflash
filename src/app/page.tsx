...
import { UserStats, VocabularyWord } from '@/lib/types';
import { A1_VOCABULARY } from '@/lib/a1-module';
import { A2_VOCABULARY } from '@/lib/a2-module';
import { B1_VOCABULARY } from '@/lib/b1-module';
...
export default function Dashboard() {
  ...
  const handleInitModule = async (module: 'A1' | 'A2' | 'B1' | 'Phrases') => {
    ...
    setTimeout(async () => {
      let data;
      if (module === 'A1') data = A1_VOCABULARY;
      else if (module === 'A2') data = A2_VOCABULARY;
      else if (module === 'B1') data = B1_VOCABULARY;
      else data = EVERYDAY_PHRASES;

      const existing = await storage.getVocabulary();
      const existingWords = new Set(existing.map(v => v.word.toLowerCase()));
      const filteredNew = data.filter(d => !existingWords.has(d.word.toLowerCase()));
  ...
  return (
    <div className="min-h-screen bg-background p-6 max-w-md mx-auto font-body flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto pb-10 no-scrollbar">
        ...
        {/* Daily Goal & Session */}
        {vocab.length > 0 && (
          <div className="bg-card p-8 rounded-[2rem] mb-10 relative overflow-hidden shadow-sm border border-accent/5">
              ...
              <Link href="/session">
                <Button size="lg" className="w-full bg-primary text-primary-foreground hover:bg-primary/90 font-bold rounded-2xl py-8 shadow-xl text-lg">
