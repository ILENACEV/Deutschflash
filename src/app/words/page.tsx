          ...
          filteredVocab.map(word => (
            <Card key={word.id} className="p-4 border-none shadow-sm bg-card flex items-center justify-between group">
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <Button variant="ghost" size="icon" className="h-8 w-8 text-primary" onClick={() => handleSpeak(word.word)} aria-label="Слушни го зборот">
                  <Volume2 className="w-4 h-4" />
                </Button>
                <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => handleDelete(word.id)} aria-label="Избриши го зборот">
